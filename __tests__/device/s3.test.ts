...
describe('S3 Device', () => {
    ...
    describe('Basic Properties', () => {
        ...
        test('should return correct description', () => {
            expect(s3Device.getDescription()).toBe('S3 Bucket Storage drive for AWS or on premise solution');
        });
    ...
    describe('Path and URL Utilities', () => {
        test('should handle absolute paths correctly', () => {
            expect(s3Device.getAbsolutePath('/path/to/../file.txt')).toBe('/path/file.txt');
            expect(s3Device.getAbsolutePath('/path/./file.txt')).toBe('/path/file.txt');
    ...
    describe('Error Handling', () => {
        test('should handle invalid credentials gracefully', async () => {
            ...
            global.fetch = jest.fn().mockRejectedValue(new Error('Authentication failed'));
    ...
    describe('Directory Operations', () => {
        ...
        test('should throw error for getPartitionTotalSpace', async () => {
            await expect(s3Device.getPartitionTotalSpace()).rejects.toThrow('Method not implemented.');
        });
    });
    ...
    describe('MIME Type Detection', () => {
        test('should detect common MIME types', async () => {
            expect(await (s3Device as any).getMimeType('test.txt')).toBe('text/plain');
            expect(await (s3Device as any).getMimeType('test.html')).toBe('text/html');
            expect(await (s3Device as any).getMimeType('test.json')).toBe('application/json');
            expect(await (s3Device as any).getMimeType('test.png')).toBe('image/png');
            expect(await (s3Device as any).getMimeType('test.jpg')).toBe('image/jpeg');
            expect(await (s3Device as any).getMimeType('test.pdf')).toBe('application/pdf');
        });

        test('should return default MIME type for unknown extensions', async () => {
            expect(await (s3Device as any).getMimeType('test.unknown')).toBe('application/octet-stream');
            expect(await (s3Device as any).getMimeType('test')).toBe('application/octet-stream');
        });
    });
});

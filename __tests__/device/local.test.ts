...
describe("Local Device", () => {
  ...
  describe("Basic Properties", () => {
    ...
    test("should return correct description", () => {
      expect(localDevice.getDescription()).toBe(
    ...
    test("should generate correct path", () => {
      ...
      const expectedPath = localDevice.getAbsolutePath(
  ...
  describe("Upload Operations", () => {
    ...
    test("should upload data (single chunk)", async () => {
      const result = await localDevice.uploadData(
        testContent,
        testFile,
        "text/plain",
      );
    ...
    test("should handle chunked upload", async () => {
      ...
      // Upload first chunk
      let result = await localDevice.uploadData(
        chunk1,
        testFile,
        "text/plain",
        1,
      ...
      // Upload second chunk
      result = await localDevice.uploadData(
        chunk2,
        testFile,
        "text/plain",
        2,
  ...
  describe("Transfer Operations", () => {
    test("should transfer file to another device", async () => {
      ...
      const targetTempDir = await fs.mkdtemp(
        path.join(os.tmpdir(), "target-device-test-"),
      );
      ...
      try {
        // Transfer file
        const result = await localDevice.transfer(
          testFile,
          targetFile,
          targetDevice,
        );
  ...
  describe("Error Handling", () => {
    test("should throw error when reading non-existent file", async () => {
      await expect(localDevice.read("/nonexistent/file.txt")).rejects.toThrow(
        "File Not Found",
      );
    });
    ...
    test("should handle upload errors gracefully", async () => {
      ...
      await expect(
        localDevice.upload("/nonexistent/source.txt", testFile),
      ).rejects.toThrow();
    });
  });
  ...
  describe("Path Utilities", () => {
    test("should resolve absolute paths correctly", () => {
      expect(localDevice.getAbsolutePath("/path/to/../file.txt")).toBe(
        "/path/file.txt",
      );
      expect(localDevice.getAbsolutePath("/path/./file.txt")).toBe(
        "/path/file.txt",
      );
      expect(localDevice.getAbsolutePath("//double//slash")).toBe(
        "/double/slash",
      );
      expect(localDevice.getAbsolutePath("/path/to/../../file.txt")).toBe(
        "/file.txt",

...
export class Local extends Device {
    ...
    async upload(source: string, filePath: string, chunk: number = 1, chunks: number = 1, metadata: Record<string, any> = {}): Promise<number> {
    ...
    async transfer(filePath: string, destination: string, device: Device): Promise<boolean> {
